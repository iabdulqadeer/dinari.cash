 ...
 * 19. Return the checkout URL of a FIAT provider
 *
 */
...
function bxc_transactions_create($amount, $cryptocurrency_code, $currency_code = false, $external_reference = '', $title = '', $description = '', $url = false, $billing = '', $vat = false, $checkout_id = false, $user_details = false, $discount_code = false, $type = 1) {
    ...
    if (bxc_crypto_is_fiat($cryptocurrency_code)) {
        ...
        return [$transaction_id, $cryptocurrency_code, bxc_checkout_url($amount, $cryptocurrency_code, $currency_code, $url, $transaction_id, $title), bxc_encryption(bxc_transactions_get($transaction_id))];
    }
    ...
    if (!$address) {
        ...
        if ($custom_token) {
            ...
            if (empty($custom_token['rate'])) {
                return bxc_error('Invalid custom token exchange rate: ' . $custom_token['rate'], 'bxc_transactions_create', true);
            }
...
function bxc_transactions_check_single($transaction) {
    ...
    if ($cryptocurrency_code === 'btc_ln') {
        ...
        if (!$confirmations && $transaction_blockchain['block_height']) {
            $confirmations = bxc_blockchain($cryptocurrency_code, 'blocks_count') - $transaction_blockchain['block_height'] + 1;
        }
...
function bxc_transactions_complete($transaction, $amount_blockchain, $address_from, $invoice = false) {
    ...
    if ($underpayment) {
        ...
        if (bxc_settings_get('accept-underpayments')) {
            ...
            $query['description'] = bxc_db_json_escape($description);
        }
    }
    ...
    if (defined('BXC_EXCHANGE') && bxc_isset($transaction, 'type') == 3) {
        ...
        $return = array_merge($return, bxc_shop_transaction_complete($transaction['customer_id'], $transaction['id'], $checkout_title, $invoice));
    }
    if (BXC_CLOUD) {
...
function bxc_transactions_update($transaction_id, $values, $complete_action = true) {
    ...
    if ($complete_action && bxc_isset($values, 'status') == 'C') {
...
function bxc_transactions_webhook($transaction, $webhook_url = false) {
    ...
    if (!bxc_transactions_webhook_authorized($transaction)) {
        return false;
    }
...
function bxc_transactions_invoice($transaction_or_id) {
    ...
    $pdf->Multicell(500, 7, bxc_('Invoice date: ') . date('d-m-Y') . PHP_EOL . bxc_('Invoice number: ') . strtoupper($prefix) . $transaction['id']);
...
function bxc_transactions_invoice_user($encrypted_transaction_id, $billing_details = false) {
    ...
    if ($transaction_id) {
        if ($billing_details) {
            ...
            $path = __DIR__ . '/uploads/inv-' . $transaction_id . '.pdf';
            if (file_exists($path)) {
                unlink($path);
            }
        }
...
function bxc_checkout_get($checkout_id = false) {
    ...
    $response = bxc_db_get('SELECT * FROM bxc_checkouts' . ($checkout_id ? ' WHERE id = ' . bxc_db_escape($checkout_id, true) : ''), $checkout_id);
    $BXC_CHECKOUTS = defined('BXC_SHOP') ? bxc_shop_checkout_get_part($response) : $response;
    return $BXC_CHECKOUTS;
}
...
function bxc_checkout_save($checkout) {
    ...
    if ($is_new) {
        return bxc_db_query('INSERT INTO bxc_checkouts(title, description, price, currency, type, redirect, hide_title, external_reference, creation_time' . $shop[0] . ') VALUES ("' . bxc_db_escape($checkout['title']) . '", "' . bxc_db_escape(bxc_isset($checkout, 'description', '')) . '", "' . bxc_db_escape($checkout['price'], true) . '", "' . bxc_db_escape(bxc_isset($checkout, 'currency', '')) . '", "' . bxc_db_escape($checkout['type']) . '", "' . bxc_db_escape(bxc_isset($checkout, 'redirect', '')) . '", ' . (empty($checkout['hide_title']) ? 0 : 1) . ', "' . bxc_db_escape(bxc_isset($checkout, 'external_reference', '')) . '", "' . gmdate('Y-m-d H:i:s') . '"' . $shop[1] . ')', true);
    } else {
        return bxc_db_query('UPDATE bxc_checkouts SET title = "' . bxc_db_escape($checkout['title']) . '", description = "' . bxc_db_escape(bxc_isset($checkout, 'description', '')) . '", price = "' . bxc_db_escape($checkout['price'], true) . '", currency = "' . bxc_db_escape(bxc_isset($checkout, 'currency', '')) . '", type = "' . bxc_db_escape($checkout['type']) . '", redirect = "' . bxc_db_escape(bxc_isset($checkout, 'redirect', '')) . '", hide_title = ' . (empty($checkout['hide_title']) ? 0 : 1) . ', external_reference = "' . bxc_db_escape(bxc_isset($checkout, 'external_reference', '')) . '"' . $shop . ' WHERE id = "' . bxc_db_escape($checkout['id'], true) . '"');
    }
}
...
function bxc_checkout_url($amount, $cryptocurrency_code, $currency_code, $checkout_url, $transaction_id, $title = '') {
    return $cryptocurrency_code == 'verifone' ? bxc_verifone_create_checkout($amount, $checkout_url, $transaction_id, $title, $currency_code) : ($cryptocurrency_code == 'stripe' ? bxc_stripe_payment(floatval($amount) * 100, $checkout_url, $transaction_id, $currency_code) : bxc_paypal_get_checkout_url($transaction_id, $checkout_url, $amount, $currency_code, $title));
}
...
function bxc_installation($data) {
    if (!defined('BXC_USER') || !defined('BXC_DB_HOST') || BXC_CLOUD) {
        ...
        if ($connection_check === true) {
            ...
            $connection->query('CREATE TABLE IF NOT EXISTS bxc_checkouts (id INT NOT NULL AUTO_INCREMENT, title VARCHAR(191), description TEXT, price VARCHAR(100) NOT NULL, currency VARCHAR(10) NOT NULL, type VARCHAR(1), redirect VARCHAR(191), hide_title TINYINT, external_reference VARCHAR(1000) NOT NULL DEFAULT "", creation_time DATETIME NOT NULL, PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
...
function bxc_version_updates() {
    if (bxc_settings_db('version') != BXC_VERSION) {
        try {
            ...
            // 09-23
            bxc_db_query('CREATE TABLE IF NOT EXISTS bxc_customers (id INT NOT NULL AUTO_INCREMENT, first_name VARCHAR(191) NOT NULL DEFAULT "", last_name VARCHAR(191) NOT NULL DEFAULT "", email VARCHAR(191), phone VARCHAR(191) NOT NULL DEFAULT "", email VARCHAR(191), country VARCHAR(191) NOT NULL DEFAULT "", country_code VARCHAR(3) NOT NULL DEFAULT "", creation_time DATETIME NOT NULL, extra TINYTEXT, PRIMARY KEY (id), UNIQUE (email)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
...
function bxc_file_delete($file_name) {
...
function bxc_stripe_create_session($price_id, $checkout_url, $client_reference_id = false) {
    ...
    $checkout_url = str_replace(['payment_status=cancelled', isset($attributes['cc']) ? 'cc=' . $attributes['cc'] : ''], '', $checkout_url);
