    ...
    var BXCAdmin = {
        ...
        repeater: {
            ...
            delete: function (i) {
                ...
                button.data('index', button_index);
                if (!is_first) {
                    parent.remove();
                }
            }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        */
        ...
        transactions_table.on('click', '.bxc-transaction-menu-btn:not(.bxc-loading)', function () {
            ...
            if (!active) {
                ...
                if (['C', 'X'].includes(status) && ((BXC_REFUNDS.includes('coinbase') && ['btc', 'eth', 'xrp', 'usdt', 'usdc', 'busd', 'bnb', 'link', 'doge', 'shib', 'ltc', 'algo', 'bat', 'bch'].includes(cryptocurrency_code)) || (BXC_REFUNDS.includes('btc') && cryptocurrency_code == 'btc') || (BXC_REFUNDS.includes('eth') && ['eth', 'usdt', 'usdc', 'link', 'shib', 'bat'].includes(cryptocurrency_code)))) {
        ...
        transactions_table.on('click', '.bxc-transaction-menu li', function () {
            ...
            if (value == 'invoice') {
                ajax('encryption', { string: id }, (response) => {
                    window.open(BXC_URL + 'pay.php?invoice=' + response);
                    loading(menu, false);
                });
            }
                ...
                for (var i = 0; i < details.length; i++) {
                    ...
                    if (value) {
                        ...
                        if (slug == 'data-hash') {
                            ...
                            value += `<div id="bxc-btn-license-key-status" class="bxc-btn-text bxc-underline" data-status="${status}" data-id="${row.find('.bxc-td-id').html()}">${bxc_(status == 1 ? 'Disable' : 'Enable')}</div>`;
                        }
        ...
        body.on('click', '#bxc-btn-license-key-status', function () {
            ...
            ajax('update-license-key-status', { transaction_id: id, status: status }, () => {
                ...
                transactions_table.find(`[data-id="${id}"]`).data('data-license-key-status', status);
        ...
        */
        ...
        main.on('click', '#checkout-downloads .bxc-repater-line i', function () {
            ...
            if (file_name) {
                ajax('delete-file', { file_name: file_name });
            }
        });

        main.on('click', '#checkout-downloads input', function () {
            if (_(this).val()) {
                window.open(BXC_URL + 'uploads/checkout/' + _(this).val());
            }
        });
        ...
        */
        ...
        if (upload_input && upload_input.e.length) {
            upload_input.e[0].addEventListener('change', function (data) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    fetch(BXC_URL + 'upload.php?target=' + _(upload_target).parent().data('upload-target'), {
                        method: 'POST',
                        body: form
                        ...
                        if (response[0]) {
                            ...
                            parent.find('input').val(response[2]);
        ...
        main.on('click', '.bxc-upload-image > i', function () {
            _(this).prev().attr('style', '');
            _(this).prev().prev().val('');
        });
