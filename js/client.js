        ...
        checkout: {
            ...
            init: function (settings, area, onSuccess = false) {
                ...
                _.ajax(BXC_URL + 'init.php', { checkout: settings, language: language, cloud: cloud }, (response) => {
                    ...
                    if (is_fiat_payment) {
                        ...
                        if (!payment_status) {
                            area.find(`.bxc-payment-methods [data-cryptocurrency="${BOXCoin.getURL('pay').toLowerCase()}"]`).e[0].click();
                            ...
                            return document.location = url + (payment_status ? (url.includes('?') ? '&' : '?') + 'payment_status=' + payment_status : '');
                        }
                    }
            ...
            vat: function (checkout) {
                ...
                if (vat.e.length) {
                    loading(vat);
                    ajax('vat', {
                        amount: checkout.attr('data-price'),
